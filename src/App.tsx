...
const Search = () => {
  ...
  const handleInputKeyDown = React.useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    const searchList = (e.target as HTMLElement).nextElementSibling;
    if (CONSTANTS.KEY.ARROW_DOWN === e.key) {
      e.preventDefault();
      const firstItem = searchList?.querySelector(searchedItemClassName);
      if (!firstItem) {
        return;
      }
      ...
      e.preventDefault();
      // if (nextSearchedItem) {
      //   (nextSearchedItem as any).click();
      // }
    }
  }, []);
  const handleSearchedItemKeyDown = React.useCallback((e: React.KeyboardEvent<HTMLButtonElement>) => {
    const targetElement = e.target as any;
    if (CONSTANTS.KEY.ARROW_UP === e.key) {
      e.preventDefault();
      const prevItem = targetElement.parentElement.previousElementSibling;
      if (prevItem === null) {
        inputRef.current?.focus();
        return;
      }
      prevItem.querySelector(searchedItemClassName).focus();
    } else if (CONSTANTS.KEY.ARROW_DOWN === e.key) {
      e.preventDefault();
      const nextItem = targetElement.parentElement.nextElementSibling;
      if (!nextItem) {
        return;
      }
      nextItem.querySelector(searchedItemClassName).focus();
    }
  }, []);
  const handleToggleSearch = () => {
    inputRef.current?.classList.toggle("-z-10");
        ...
        {products.map((product) => (
          <li key={product.id}>
            <button
              ...
              onKeyDown={handleSearchedItemKeyDown}
          ...
          <div className="fixed z-10 w-full navbar shadow-lg bg-white dark:bg-neutral text-neutral-content">
            <div className="flex w-full xl:container xl:m-auto">
              ...
              <div className="flex-none hidden md:flex md:flex-1 ml-2">
                ...
                <Link className="btn btn-ghost btn-sm rounded-btn text-gray-700 dark:text-white" to="/accessories">
                  액세서리
                </Link>
              ...
              <div className="flex items-center px-2">
                <label className="swap swap-rotate mr-2 sm:mr-4">
                  <input
                    type="checkbox"
                    className="js-theme"
                    checked={theme === CONSTANTS.THEME.LIGHT}
                    onChange={(e) => {
                      const newTheme = e.target.checked ? CONSTANTS.THEME.LIGHT : CONSTANTS.THEME.DARK;
                      setTheme(newTheme);
                    }}
                  />
